...
def gen_par(fs,T_obs,mdist='astro',beta=[0.75,0.95],verbose=True):
    ...
    # store params
    # 注意: 原版 GWData-Bootcamp 此处存的是 np.cos(iota) 而非 iota 本身，
    # 但 SimInspiralChooseTDWaveform 期望的是倾角 iota (弧度)。
    # 这里修正为存真实倾角。
...
def gen_bbh(fs,T_obs,psds,snr=1.0,dets=['H1'],beta=[0.75,0.95],par=None,verbose=True):
    ...
    # f_low now holds the actual frequency used for successful generation
